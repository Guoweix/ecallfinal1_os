...
corrupted:
	EXT4_Debug_s_i64(DEBUG_EXTENT, "Bad extents B+ tree block: %s. "
			       "Blocknr: %" PRId64 "\n",
		 error_msg, pblk);
	return EIO;
}
				   ...
				   ext4_lblk_t to)
{
	...
	EXT4_Debug_u32_u64_u32(DEBUG_EXTENT,
			       ...
			       struct ext4_extent_path *path, int32_t depth)
{
	...
	EXT4_Debug_u32_u64_i32(DEBUG_EXTENT, "IDX: Freeing %" PRIu32 " at %" PRIu64 ", %d\n",
...
__unused static void print_path(struct ext4_extent_path *path)
{
	...
	while (i >= 0) {
		...
		(void)a;
		(void)b;
		EXT4_Debug_i32_u64_td_td(DEBUG_EXTENT,
