...
void SlabB::Init(Uint32 size, Uint32 Pagecounts,Uint32 Usage)
{
    using namespace POS;
    kout[MemInfo] << "SlabB init..." << endl;
    // 存储头结点
    slabB* s = (slabB*)slab.allocateSlab(Pagecounts * 4096, Usage); // 一页的虚拟起始地址
    ...
    PagesEndAddr = s + slabBCount; // 总共页表头的地址
...
bool SlabB::freeslabB(slabB* t)
{
    ...
    t->flags = 0; // 空闲
    slabB* pre = head.nxt;
    if (pre == nullptr || pre > t) { // 释放前，已经没有空闲页或者释放的是nxt前的空闲页
        slabB* headnxt = pre;
        head.nxt = t; // 添加到头结点后面
        t->pre = &head;
        t->nxt = headnxt;
