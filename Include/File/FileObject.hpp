...
public:
    // 链表结构相关
    void init_proc_fo_head(Process* proc);                  // 专门生成并初始化进程的fo头节点
    ...
    // 文件处理相关
    Sint64 read_fo(file_object* fo, void* dst, Uint64 size);     // 在fo中从当前文件指针的位置读size大小内容到dst
    Sint64 write_fo(file_object* fo, void* src, Uint64 size);    // 在fo中写size大小的内容到文件的当前文件指针位置中
    void seek_fo(file_object* fo, Sint64 bias,                   // 移动文件指针的位置 默认相对于文件头进行移动
