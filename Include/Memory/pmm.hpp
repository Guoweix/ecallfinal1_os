...
struct PAGE
{
   Uint64 flags,//0表示空闲，1表示不是slab，2表示slab64B，3表示slab512B，4表示slab4KB
	...
	{return ((Uint64)this-FreeMemoryStart())/sizeof(PAGE);}//第几页
	
	inline void* KAddr() const
	{return (void*)(FreeMemoryStart()+Index()*PAGESIZE);}//具体页的地址
	
	inline void* PAddr() const
	{return (void*)(FreeMemoryStart()+Index()*PAGESIZE-PVOffset);}//物理地址
