...
void FileNode::set_parent(FileNode* _parent)
{
    if (_parent == nullptr)
    {
        kout[Fault] << "FileNode ptr is null" << endl;
        return;
    }

    if (_parent->IsDir())
    {
        kout[Fault] << "FileNode isn't a Dir" << endl;
        return;
    }
...
char* FileNode::get_path(Uint8 _flag)
{
    ...
    if (len == 0)
    {
        char* re = (char*)kmalloc(2 * sizeof(char));
        strcpy(re, "/");
        return re;
    }
    else
    {
        char* re = (char*)kmalloc(len * sizeof(char) + 1);
